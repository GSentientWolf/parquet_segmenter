...
from typing import (
    Any, Callable, Iterator, Protocol, runtime_checkable,
    Tuple, Optional, List, TYPE_CHECKING
)
import sys
...
class BaseSegmenter:
    ...
    def _normalize_table(self, table: Any) -> Any:
        ...
        if (pa is not None and hasattr(pa, 'Table') and
                isinstance(table, pa.Table)):
            return table

        # If pandas present and table is a DataFrame, allow it through
        if (pd is not None and hasattr(pd, 'DataFrame') and
...
class MaxSizeBasedStrategy(BaseSegmenter):
    ...
    enable_backtracking: bool = True  # Enable backtracking optimization

    def segment(self, table: Any,
                oversized_handler: Optional[Callable[[List[Any]], Any]] = None,
                **kwargs: Any) -> SegmentResult:
        ...
        Args:
            ...
            oversized_handler: Optional callable to handle oversized segments.
                              If provided, will be called with each oversized
                              segment. If not provided, oversized segments are
        ...
        if (pd is not None and hasattr(pd, 'DataFrame') and
            ...
            try:
                ...
                if oversized and oversized_handler is not None:
                    logger.info("Processing %d oversized segments with handler",
                                len(list(oversized)))
                    try:
                    ...
                    logger.warning("Found %d oversized segments that exceed "
                                   "size limit", len(list(oversized)))
        ...
        elif (pa is not None and hasattr(pa, 'Table') and
              isinstance(table, pa.Table)):
            try:
                segments, oversized = self._segment_pyarrow_table(table)

                if oversized and oversized_handler is not None:
                    logger.info("Processing %d oversized PyArrow segments with provided handler", len(list(oversized)))
                    try:
                        oversized_handler(list(oversized))
                    except Exception as e:
                        logger.error("Error in oversized_handler: %s", e)
                else:
                    logger.warning("Found %d oversized PyArrow segments that exceed "
                                   "size limit", len(list(oversized)))
    ...
    def _segment_dataframe(self, df: Any) -> Tuple[SegmentResult, SegmentResult]:
        """Segment a pandas DataFrame using iterative divide-and-conquer.

        Uses a deque-based algorithm to partition the DataFrame into segments
        where each segment is guaranteed to be < max_size_limit bytes. The
        algorithm recursively divides DataFrame chunks that exceed the size
        limit. If enable_backtracking is True, applies backtracking to merge
        adjacent segments when possible for optimal segmentation.

        Args:
            df: The pandas DataFrame to segment

        Returns:
            Tuple of (normal_segments, oversized_segments) as SegmentResult
        """
        if len(df) == 0:
        ...
        while work_queue:
            ...
            if chunk_size <= self.max_size_limit:
                ...
                if self.enable_backtracking:
                    # Store as (start_idx, end_idx, chunk, size) for backtrack
                    segments.append((start_idx, end_idx, chunk.copy(),
                                     chunk_size))
                else:
                ...
                if end_idx - start_idx <= 1:
                    ...
                    logger.warning(
                        "Single row DataFrame segment exceeds size limit: "
                        "%d bytes > %d bytes",
                    ...
                    # Add the two halves to the front of deque for depth-first
                    # processing. This tends to create more balanced segments
                    work_queue.appendleft((mid_idx, end_idx))  # Second half
                    work_queue.appendleft((start_idx, mid_idx))  # First half

        # Phase 2: Backtracking to merge adjacent segments when possible
        if self.enable_backtracking and segments:
            ...
            final_segments = [seg[2] if isinstance(seg, tuple) else seg
                              for seg in segments]
        else:
            # No backtracking - segments are already DataFrames or need extract
            final_segments = [seg[2] if isinstance(seg, tuple) else seg
                              for seg in segments]

        return (ListSegmentResult(final_segments),
                ListSegmentResult(over_sized_segments))
    ...
    def _apply_backtracking(self, df: Any, segments: list) -> list:
        ...
        while i < len(segments):
            ...
            merged = False
            if i + 1 < len(segments):
                next_start, next_end, _, _ = segments[i + 1]
                ...
                if current_end == next_start:
                    ...
                    if merged_size <= self.max_size_limit:
                        ...
                        optimized_segments.append((current_start, next_end,
                                                   merged_chunk.copy(),
                                                   merged_size))
                        merged = True
                        ...
                        logger.debug(
                            "Backtracking: merged segments [%d:%d] + [%d:%d] "
                            "= [%d:%d] (%d bytes)",
            ...
            if not merged:
                ...
                optimized_segments.append((current_start, current_end,
    ...
    def _apply_backtracking_pyarrow(self, table: Any, segments: list) -> list:
        """Apply backtracking to merge adjacent PyArrow Table segments when possible.

        Args:
            table: The original PyArrow Table
            segments: List of (start_idx, end_idx, chunk, size) tuples

        Returns:
            List of optimized segments (either tuples or PyArrow Tables)
        """
        if len(segments) <= 1:
            return segments
        ...
        while i < len(segments):
            ...
            if i + 1 < len(segments):
                ...
                if current_end == next_start:
                    # Calculate combined size by creating the merged segment
                    merged_chunk = table.slice(current_start, next_end - current_start)
                    merged_size = merged_chunk.nbytes

                    if merged_size <= self.max_size_limit:
                        # Merge successful - create new merged segment
                        optimized_segments.append((current_start, next_end, merged_chunk, merged_size))
                        merged = True
                        i += 2  # Skip the next segment as it's been merged

                        # Log the merge for debugging
                        logger.debug(
                            "Backtracking: merged PyArrow segments [%d:%d] + [%d:%d] = [%d:%d] (%d bytes)",
                            current_start, current_end, next_start, next_end,
                            current_start, next_end, merged_size
                        )
        ...
        if len(optimized_segments) < len(segments):
            # We made progress, try another round of merging
            return self._apply_backtracking_pyarrow(table, optimized_segments)
        else:
            # No more merging possible
            return optimized_segments

    def _segment_pyarrow_table(self, table: Any) -> Tuple[SegmentResult, SegmentResult]:
        """Segment a PyArrow Table using iterative divide-and-conquer with optional backtracking.

        Uses a deque-based algorithm to partition the PyArrow Table into segments where
        each segment is guaranteed to be < max_size_limit bytes. The algorithm
        recursively divides table chunks that exceed the size limit. If
        enable_backtracking is True, applies backtracking to merge adjacent segments
        when possible for optimal segmentation.

        Args:
            table: The PyArrow Table to segment

        Returns:
            Tuple of (normal_segments, oversized_segments) as SegmentResult objects
        """
        if len(table) == 0:
            raise ValueError("Cannot segment an empty PyArrow Table")

        segments = []
        over_sized_segments = []
        # Initialize deque with the full table range (start_idx, end_idx)
        work_queue = deque([(0, len(table))])
        ...
        while work_queue:
            ...
            if chunk_size <= self.max_size_limit:
                ...
                if self.enable_backtracking:
                    ...
                    segments.append((start_idx, end_idx, chunk, chunk_size))
                else:
                    # Store just the chunk for non-backtracking mode
                ...
                if end_idx - start_idx <= 1:
                    # Cannot divide further (single row exceeds limit)
                    over_sized_segments.append(chunk)
                    logger.warning(
                        "Single row PyArrow Table segment exceeds size limit: %d bytes > %d bytes",
                        chunk_size, self.max_size_limit
                    )
                else:
                    # Split the chunk in half and add both halves back to queue
        ...
        if self.enable_backtracking and segments:
            segments = self._apply_backtracking_pyarrow(table, segments)
            # Extract just the Tables from the segment tuples
            final_segments = [seg[2] if isinstance(seg, tuple) else seg
                              for seg in segments]
        else:
            # No backtracking - segments are already Tables or need extraction
            final_segments = [seg[2] if isinstance(seg, tuple) else seg
                              for seg in segments]
    ...
    def _segment_generic(self, data: Any) -> SegmentResult:
        ...
        try:
            ...
            if hasattr(data, '__len__'):
                ...
                bytes_per_item = (data_size / total_items
                                  if total_items > 0 else 1)
                items_per_segment = max(1, int(self.max_size_limit /
                                               bytes_per_item))
            ...
            # Single item - return as is if within limit
            return ListSegmentResult([data])
