...
class BaseSegmenter:
    ...
    def _normalize_table(self, table: Any) -> Any:
        ...
        if pa is not None and hasattr(pa, 'Table') and isinstance(table, pa.Table):
            return table

        # If pandas present and table is a DataFrame, allow it through
        if pd is not None and hasattr(pd, 'DataFrame') and isinstance(table, pd.DataFrame):
            return table
...
class MaxSizeBasedStrategy(BaseSegmenter):
    ...
    def segment(self, table: Any, oversized_handler: Optional[Callable[[List[Any]], Any]] = None, **kwargs: Any) -> SegmentResult:
        ...
        elif pa is not None and hasattr(pa, 'Table') and isinstance(table, pa.Table):
            try:
